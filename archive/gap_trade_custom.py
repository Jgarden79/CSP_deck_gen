...
class GapTrade():
    ...
    '''
    Class object to calculate and visualize Gap Trades.
    ...
    :param gap: <int> gap between current market price and start of protection; if 0, no gap
    :param prot: <int> target protection (difference between where protection starts and where protection ends)
    ...
    ..........
        ...
        gap : int
            Gap between current market price and start of protection; if 0, no gap

        prot : int
            Target protection defined as the percentage between the Long Call strike and the Short Put strike
    ...
    def __init__(self,RIC,get_data,expiry,gap,prot,exclude_div,custom_div='n',tgt_yld=0,fee=0.0125):
    ...
    def __repr__(self):
        return "Cap_and_Cushion(RIC={},get_data={},expiry={},gap={},prot={}),exclude_div={},custom_div={},fee={}".format(
                    self.RIC,
                    self.get_data,
                    self.expiry,
                    self.gap,
    ...
    def calc_trade(self,output='line',firm='L'):
        ...
        trade_line = pd.DataFrame({"Underlying Asset": self.sym,
                                   ...
                                   'Downside Before Protection (%)': '{:.2%}'.format(self.ds_before),
        ...
        self.cost = self.LPUT['MID'] - self.SCALL['MID'] - self.SPUT['MID'] + self.last  # total trade cost including underlying
        ...
        if output == 'line':  # line output
            ...
            # Figure objects
            trade_title = f"{self.sym} Cap &<br>Cushion Trade"
            trade_subtitle = f"Ending {fc_date}"
            table_description = f"The investor gets about 100% of the price appreciation on {self.sym} beginning at a price<br>"\
                f"of {int(self.last)}, capped at {int(self.SCALL['STRIKE_PRC'])} and full protection between "\
                f"{int(self.LPUT['STRIKE_PRC'])} and {int(self.SPUT['STRIKE_PRC'])} ({-self.protection:.2%})"
            ...
            trade_summary_data = [
                ...
                ('Cap', f"{self.upside:.2%}"),
                ('Downside Before Protection', f"{self.ds_before:.2%}"),
            ...
            pio.write_html(fig, file='html/{}_{}_{}_cap_gap_custom.html'.format(self.sym, self.expiry, self.firm),
                           auto_open=False)
            pio.write_image(fig, file='Images/{}_{}_{}_cap_gap_custom.png'.format(self.sym, self.expiry, self.firm),format='png', scale=6)
            prs = Presentation('rebrand_test.pptx')
            prs.save('Images/{}_{}_{}_cap_gap_custom.pptx'.format(self.sym, self.expiry, self.firm))
            shapes = prs.slides[0].shapes
            pic = shapes.add_picture('Images/{}_{}_{}_cap_gap_custom.png'.format(self.sym, self.expiry, self.firm), Inches(0.001), Inches(0.001),
    ...
    def payoff_plot(self):
        ...
        # plt.annotate('{:.1%} Downside Before Protection'.format(gap),xy=(long_put,gap),xytext=(150,-50),
        #         textcoords='offset points',arrowprops=dict(facecolor='navy',shrink=0.05),
        #         size=12,color='navy')
        # plt.annotate('{:.1%} Downside Protection'.format(protection),xy=(short_put,0),xytext=(-150,50),
        #         textcoords='offset points',arrowprops=dict(facecolor='navy',shrink=0.05),
        #         size=12,color='navy')
        # plt.annotate('Upside Capped at {:.1%}'.format(self.cap),xy=(short_call,self.cap),xytext=(-150,50),
        #         textcoords='offset points',arrowprops=dict(facecolor='navy',shrink=0.05),
        #         size=12,color='navy')
        ...
        plt.tight_layout()
        plt.savefig('images/{}_gap_trade_payoff_plot.png'.format(self.sym))
    ...
    def data_table(self):
        ...
        df=pd.DataFrame({'return':ret,'cap_cush':strat_pl_5/self.port_val,'cap_cush_net':strat_pl_5_net/self.port_val,'stock':stock_pl_5/self.port_val})
        ...
        ws3['A36'] = self.last * self.shares
        ws3['A38'] = self.net_opt_cost_dlrs * self.contracts
        ws3['A40'] = self.upside
        ws3['A42'] = self.ds_before
        ws3['A44'] = self.protection #days until CC expiration
        ws3['A46'] = '{:.1f} months'.format(self.adj_time_left / 30) #months left
        ws3['A48'] = abs(self.trade_delta) #add trade delta
        ws4=wb.worksheets[3] #define worksheet
        ws4['A15'] = f'{self.sym} with Lido CSP (gross)'
        ws4['A16'] = f'{self.sym} with Lido CSP (net)'
        ws4['A17'] = self.sym
        # #append covered call returns table
        for row in np.arange(0,len(df),1):
            for col in np.arange(0,len(df.columns),1):
                ws4.cell(row=row+15,column=col+2).value=df.iloc[row,col]
    ...
    def historical_chart(self,period='3y'):
        ...
        plt.tight_layout()
        plt.savefig('images/{}_{}r_price_chart_CapCush.png'.format(self.sym,period))
        plt.show();
    ...
    def payoff_plot_plain(self):
        ...
        plt.title("Lido Cap & Cushion Strategy",fontsize=14)
